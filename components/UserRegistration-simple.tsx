...
import { User, MapPin, FileText, Calendar, ChevronRight } from 'lucide-react';
...
export function UserRegistration() {
  ...
  const handleSubmit = async () => {
    setLoading(true);
    try {
      // Save registration to localStorage using the wallet address
      if (currentAccount) {
      ...
      await new Promise<void>((resolve, reject) => {
        registerUser(
          ...
          formData.location,
          () => {
            // Mark as registered on blockchain
            if (currentAccount) {
              localStorage.setItem(`blockchain_registered_${currentAccount}`, 'true');
            }
            resolve();
            router.push('/swipe');
          },
      ...
      if (error.message && error.message.includes('already registered')) {
        alert('You are already registered! Redirecting to dating page...');
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-primary/5 to-accent/5 flex items-center justify-center p-4">
      <div className="w-full max-w-md">
        <div className="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 relative">
          {/* Progress Bar */}
          <div className="mb-8">
